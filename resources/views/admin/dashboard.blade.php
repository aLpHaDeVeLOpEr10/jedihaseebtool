...
@section('content')
{{-- Stats Cards --}}
<div class="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-6 sm:mb-8">
    <div class="stat-card">
        <div class="flex items-center justify-between mb-3">
            <p class="text-xs sm:text-sm font-medium text-gray-500 truncate pr-2">Total Tools</p>
        ...
        <p class="text-2xl sm:text-3xl font-bold text-gray-900">{{ number_format($stats['total_tools']) }}</p>
        <p class="text-xs text-emerald-600 mt-1">{{ $stats['active_tools'] }} active</p>
    </div>
    <div class="stat-card">
        <div class="flex items-center justify-between mb-3">
            <p class="text-xs sm:text-sm font-medium text-gray-500 truncate pr-2">Categories</p>
        ...
        <p class="text-2xl sm:text-3xl font-bold text-gray-900">{{ $stats['total_categories'] }}</p>
        <p class="text-xs text-gray-400 mt-1">tool categories</p>
    </div>
    <div class="stat-card">
        <div class="flex items-center justify-between mb-3">
            <p class="text-xs sm:text-sm font-medium text-gray-500 truncate pr-2">Total Views</p>
        ...
        <p class="text-2xl sm:text-3xl font-bold text-gray-900">{{ number_format($stats['total_views']) }}</p>
        <p class="text-xs text-gray-400 mt-1">page views</p>
    </div>
    <div class="stat-card">
        <div class="flex items-center justify-between mb-3">
            <p class="text-xs sm:text-sm font-medium text-gray-500 truncate pr-2">Unread Messages</p>
        ...
        <p class="text-2xl sm:text-3xl font-bold text-gray-900">{{ $stats['unread_contacts'] }}</p>
...
<div class="grid lg:grid-cols-3 gap-4 sm:gap-6">
    ...
    <div class="lg:col-span-2 card overflow-hidden">
        ...
        <div class="divide-y divide-gray-50">
            ...
            <div class="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 hover:bg-gray-50 transition-colors">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs text-gray-400 truncate">{{ $tool->category->name }} · {{ $tool->created_at->diffForHumans() }}</p>
                </div>
                <div class="flex items-center gap-2 flex-shrink-0">
                    <span class="hidden sm:inline-flex badge {{ $tool->status === 'active' ? 'badge-success' : 'badge-gray' }} text-xs">
                        {{ $tool->status }}
                    </span>
