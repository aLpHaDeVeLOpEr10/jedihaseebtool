...
<div x-data="{ activeTab: 'basic' }">
    ...
    <div class="flex gap-1 mb-6 border-b border-gray-200">
        @foreach([['basic', 'Basic Info'], ['seo', 'SEO'], ['inputs', 'Inputs & FAQs'], ['blade', 'Blade Template']] as [$tab, $label])
    ...
    <form action="{{ route('admin.tools.update', $tool) }}" method="POST" class="space-y-6">
        @csrf @method('PUT')
        ...
        <div x-show="activeTab === 'basic'" class="grid lg:grid-cols-3 gap-6">
            ...
            <div class="space-y-6">
                ...
                <div class="card p-5">
                    ...
                    @if(!$bladeExists)
                    <form action="{{ route('admin.tools.generate-blade', $tool) }}" method="POST" class="mt-3">
                        @csrf
        ...
        <div x-show="activeTab === 'seo'" x-cloak class="card p-6 max-w-2xl">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="seo_title" value="{{ old('seo_title', $tool->getOriginal('seo_title')) }}"
                           class="form-input" placeholder="Leave blank to auto-generate">
                </div>
                <div>
                    <label class="form-label">Meta Description</label>
                    <textarea name="seo_description" rows="3" class="form-input">{{ old('seo_description', $tool->getOriginal('seo_description')) }}</textarea>
                </div>
                <div>
                    <label class="form-label">SEO Keywords</label>
                    <input type="text" name="seo_keywords" value="{{ old('seo_keywords', $tool->seo_keywords) }}"
        ...
        <div x-show="activeTab === 'blade'" x-cloak class="card p-6">
            <div class="flex items-center justify-between mb-5">
                ...
                @if(!$bladeExists)
                <form action="{{ route('admin.tools.generate-blade', $tool) }}" method="POST">
                    @csrf
