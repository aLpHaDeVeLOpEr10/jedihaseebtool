...
<div class="card p-4 mb-6">
    <form action="{{ route('admin.tools.index') }}" method="GET"
        ...
        <div class="flex gap-2 sm:col-span-2 xl:col-span-1">
            <button type="submit" class="btn btn-primary flex-1 xl:flex-none">Filter</button>
            <a href="{{ route('admin.tools.index') }}" class="btn btn-secondary flex-1 xl:flex-none">Clear</a>
        </div>
    </form>
</div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 border-b border-gray-100">
                <tr>
                    ...
                    <th class="hidden md:table-cell text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Category</th>
                    <th class="hidden lg:table-cell text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Type</th>
                    <th class="hidden sm:table-cell text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Status</th>
                    <th class="hidden lg:table-cell text-right px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Views</th>
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50 transition-colors {{ $tool->trashed() ? 'opacity-50' : '' }}">
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div class="min-w-0">
                                <div class="flex items-center gap-2 flex-wrap">
                                    <p class="font-medium text-gray-900 break-words">{{ $tool->name }}</p>
                                    @if($tool->is_featured)
                                ...
                                <p class="text-xs text-gray-400 font-mono break-all">{{ $tool->slug }}</p>
                                {{-- Columns hidden at this width are folded in here. --}}
                                <p class="lg:hidden text-xs text-gray-400 mt-1">
                                    <span class="md:hidden">{{ $tool->category->name ?? '—' }} · </span>
                                    <span class="capitalize">{{ $tool->tool_type }}</span>
                    ...
                    <td class="hidden md:table-cell px-4 py-3 text-gray-600">
                        {{ $tool->category->name ?? '—' }}
                    </td>
                    ...
                    <td class="hidden lg:table-cell px-4 py-3 text-right text-gray-600">
